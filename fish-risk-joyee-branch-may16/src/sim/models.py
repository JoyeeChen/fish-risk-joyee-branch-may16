...
class DecayingPreferenceModel(Model):

    def __call__(self):
        for t_sim in range(self.horizon):
            Lt_sim, Vt_sim, Bt_sim, self.params = self.timestep(t_sim, self.params)
            #print("Before decay: w = ", self.params.w)
            self.params = self.params._replace(w = self.params.w/2.0)
            #print("After halving: w = ", self.params.w)
            sim_params = self.get_sim_params()
            #print("sim_params' w:", sim_params.w)
            Rt_sim = self.plan(t_sim, sim_params)
            print(Rt_sim)

class DecayingPreferenceModelWithLogging(Model):
    def plan(self, t_sim, params):
        Rt_sim = 0.
        for t_plan in range(self.horizon):
            t = t_sim + t_plan
    ...
    def __call__(self):
        for t_sim in range(self.horizon):
            ...
            self.params = self.params._replace(w = self.params.w/2.0)
            sim_params = self.get_sim_params()
            Rt_sim, min_Gt = self.plan(t_sim, sim_params)
            print("Rt_sim:", Rt_sim, "and min_Gt:", min_Gt)
            R_to_min_G_ratio = Rt_sim[0]/min_Gt[0]
            return R_to_min_G_ratio
